...
- **State government trifectas (2012–2024)** — a bundled dataset in
  ...
  **governance year** (control in effect *during* that calendar year, i.e. the result of
  ...
  same year.
- **Unemployment — BLS Local Area Unemployment Statistics (2012–2023)** — a bundled dataset
  in `src/data/laus.json`, generated by `scripts/build-laus.mjs` from the
  [BLS public API](https://www.bls.gov/developers/). It stores the official annual-average
  unemployment rate for every state and county, keyed by FIPS and year. This is the
  authoritative labor measure and sits beside the ACS survey estimate for comparison.
  Regenerating it needs a free [BLS API key](https://data.bls.gov/registrationEngine/) in
  `BLS_API_KEY` (see the script header); the committed `laus.json` means the app itself
  needs no key.
- **Wages — BLS Quarterly Census of Employment and Wages (2014–2023)** — a bundled dataset
  in `src/data/wages.json`, generated by `scripts/build-qcew-wages.mjs` from the
  [QCEW Open Data API](https://www.bls.gov/cew/) (no key, one request per year). It stores
  the average annual pay (mean pay per covered job) for every state and county — a
  different economic lens than ACS median household income. The QCEW API's earliest year is
  2014, so 2012–2013 show as no data for this factor.
- **Life expectancy — [County Health Rankings & Roadmaps](https://www.countyhealthrankings.org/)
  (2019–2023)** — a bundled dataset in `src/data/health.json`, generated by
  `scripts/build-chr-health.mjs` from CHR's annual analytic data files (one request per
  year, columns matched by name since their positions shift between releases). County and
  state life expectancy in years — a longevity lens for correlation against income,
  education and wages. Published from CHR's 2019 release onward, so earlier slider years
  show as no data.
- **Cost of living — [BEA Regional Price Parities](https://www.bea.gov/data/prices-inflation/regional-price-parities-state-and-metro-area)
  (2008–2024)** — a bundled dataset in `src/data/rpp.json`, generated by
  `scripts/build-rpp.mjs` from the U.S. Bureau of Economic Analysis's keyless zip download.
  A price index (US = 100) broken into components — all items, goods, housing, utilities,
  and other services — so you can ask "are high-income states just expensive?" RPP is
  published at state level only (not per county), so these factors show no data when
  drilled into a state's counties.
